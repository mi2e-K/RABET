...
class ConfigManager:
    ...
    def _migrate_old_keys(self, config):
        ...
        """
        try:
            # Check for old directory key
            if "directories" in config:
                if "last_action_map_directory" in config["directories"]:
                    # Copy value to new key if the new key is empty
                    if not config["directories"].get("last_config_directory"):
                        config["directories"]["last_config_directory"] = config["directories"]["last_action_map_directory"]
                    
                    # Remove old key
                    del config["directories"]["last_action_map_directory"]
                    self.logger.info("Migrated last_action_map_directory to last_config_directory")
            
            # Check for old recent files key
            if "recent_files" in config:
                if "action_maps" in config["recent_files"]:
                    # Copy value to new key if the new key is empty
                    if not config["recent_files"].get("configs"):
                        config["recent_files"]["configs"] = config["recent_files"]["action_maps"]
                    
                    # Remove old key
                    del config["recent_files"]["action_maps"]
                    self.logger.info("Migrated recent_files.action_maps to recent_files.configs")
        except Exception as e:
            self.logger.warning(f"Error migrating old keys: {str(e)}")
    ...
    def add_recent_file(self, file_type, file_path):
        ...
        Args:
            file_type (str): Type of file (videos, configs, annotations)
            file_path (str): Path to the file
        
        Returns:
            bool: True if the file was added successfully, False otherwise
        """
        # Map old file_type 'action_maps' to 'configs'
        if file_type == 'action_maps':
            file_type = 'configs'
            self.logger.debug("Mapped file_type 'action_maps' to 'configs'")
    ...
    def update_last_directory(self, directory_type, directory_path):
        ...
        Args:
            directory_type (str): Type of directory (video, annotation, config)
            directory_path (str): Path to the directory
        
        Returns:
            bool: True if the directory was updated successfully, False otherwise
        """
        # Map old directory_type 'action_map' to 'config'
        if directory_type == 'action_map':
            directory_type = 'config'
            self.logger.debug("Mapped directory_type 'action_map' to 'config'")
