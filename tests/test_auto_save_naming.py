...
def test_video_at_a_filesystem_root_falls_back_to_the_bare_name():
    """A video at the root has no parent folder to qualify with.

    The root is taken from the running platform ("C:\\" on Windows, "/"
    elsewhere) rather than hard-coded: a Windows drive letter is just a
    directory name to posixpath, so a literal "D:\\" would be qualified as
    "D_" on Linux and the test would fail there while the code is correct on
    the platform that actually sees such paths.
    """
    root = os.path.abspath(os.sep)
    video = os.path.join(root, "trial.mp4")
    assert os.path.dirname(video) == root

    assert stem(_Dummy(), video, os.path.join(root, "out"), root) == "trial"
