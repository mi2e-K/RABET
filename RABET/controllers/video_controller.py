...
class VideoController(QObject):
    ...
    def handle_step_forward(self, time_ms=100):
        ...
        if self._stepping_in_progress or self._video_initializing:
            self.logger.debug("Step forward request ignored - operation in progress")
            return
        ...
        try:
            ...
            # Force smaller steps to reduce precision issues
            if actual_step_ms > 200:
                # For larger steps, break into multiple smaller steps for better precision
                self.logger.debug(f"Breaking large step of {actual_step_ms}ms into multiple smaller steps")
                # Schedule multiple steps with delays between them
                self._perform_multiple_forward_steps(actual_step_ms)
                return
            
            # Log the step operation
            self.logger.debug(f"Performing step forward of {actual_step_ms}ms")
            ...
            # Calculate target position precisely
            target_pos = before_pos + actual_step_ms
            
            # Perform the step by direct seeking
