...
class AnnotationController(QObject):
    ...
    def _auto_export_annotations(self):
        """Automatically export annotations to the same location as the video."""
        if not self._current_video_path or not self._annotation_model.get_all_events():
            return
        
        try:
            # Generate export path based on video path
            ...
            # Check if file already exists - add timestamp to avoid overwriting
            ...
            if self._annotation_model.export_to_csv(export_path, include_header=True):
                self._main_window.set_status_message(f"Annotations exported to {export_path}")
                self.logger.info(f"Annotations automatically exported to {export_path}")
                ...
                self.logger.error(f"Failed to export annotations to {export_path}")
        except Exception as e:
    ...
    def export_annotations_dialog(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self._main_window, "Export Annotations", "", "CSV Files (*.csv)"
        )
    ...
    def import_annotations_dialog(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self._main_window, "Import Annotations", "", "CSV Files (*.csv)"
        )
